...
def mark_clip(file_name):
    ...
    clip = mpy.VideoFileClip(f"./saved_clips/{file_name}")  # .subclip(0,1)
    ...
    final  = mpy.CompositeVideoClip([clip, text_bg, twitch_logo, streamer_name], size = (1920,1080))

    # final.ipython_display()
    # final_clip = concatenate(clip_list, method = "compose")

    final.write_videofile(f"./edited_clips/{file_name}", fps = 30)
...
def montage(clips=[], transition=None, max_duration=9999, title=""):
    ...
    while duration <= max_duration:
        for clip in clips:
            ...
            temp_clip = mpy.VideoFileClip(f"./edited_clips/{clip}")
